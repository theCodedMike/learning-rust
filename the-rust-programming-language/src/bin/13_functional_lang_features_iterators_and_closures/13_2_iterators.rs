...
///
/// ### 消费迭代器的方法
/// - 消费型适配器(consuming adaptors): 调用 next 方法的方法，他们会消耗迭代器。如sum()
///
/// ### 产生其他迭代器的方法
/// - 迭代型适配器(iterator adaptors): 可以将当前迭代器变为不同类型的迭代器。如map()
///
/// ### 使用捕获其环境的闭包
///
fn main() {
    ...
    // IntoIterator 强调的是某一个类型如果实现了该特征，它可以通过 into_iter，iter等方法变成一个迭代器
    let names = ["zhang fei", "guan yu", "liu bei"];
    let ages = [45, 47];
    let folks: HashMap<_, _> = names.into_iter().zip(ages.into_iter()).collect();
    println!("collect: {:?}", folks); //{"zhang fei": 45, "guan yu": 47}
}
